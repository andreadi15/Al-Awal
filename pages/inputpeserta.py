...
class InputPesertaPage(ctk.CTkFrame):
    ...
    def collect_form(self):
        while True:
            ...
            if not DB_Get_Peserta_By_Id(id_peserta):
                break
        if "<pilih skema>" in self.entries["skema"].get().strip().lower():
            skema = ""
        else:
    ...
    def load_form(self, peserta: PesertaModel):
        if not peserta.skema:
            self.entries["skema"].set("<Pilih Skema>")
        else:
        ...
        if not peserta.pendidikan:
            self.entries["pendidikan"].set("<Pilih Pendidikan>")
        else:
    ...
    def clear_form(self):
        ...
        for key, widget in self.entries.items():
            ...
            try:
                widget.delete(0, "end")
                widget.focus_set()
                self.entries["sertifikasi"].focus_set()
            except Exception:
                pass
    ...
    def restore_state(self):
        ...
        try:
            ...
            if saved_id and saved_id == self.selected_id_sertifikasi:
                ...
                current_form = [PesertaModel.from_dict(p) for p in state.get('current_form_data', {})]
                if current_form:
    ...
    def _load_form_data(self, form_data: PesertaModel):
